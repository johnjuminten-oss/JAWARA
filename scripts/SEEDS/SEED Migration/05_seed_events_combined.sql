-- First ensure the event_type enum is updated
BEGIN;
...
-- Now proceed with the seed data
BEGIN;
...
weekly_schedule(week_no, schedule_date, day_name, start_time, end_time, subject, teacher) AS (
  VALUES
  -- Keep all your existing VALUES here
  -- WEEK 1
...
SELECT
  ...
  CASE
    ...
    -- Default to regular study for unmatched subjects
    ELSE 'regular_study'
  ...
  jsonb_build_object(
    ...
    'color', CASE 
      ...
      WHEN n.subject ILIKE 'Sholat%' OR n.subject ILIKE '%Agama%' OR n.subject ILIKE 'Literasi Kitab%' THEN '#20C997'  -- Teal for prayer
