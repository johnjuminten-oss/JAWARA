...
export function StudentDashboardContent({ profile, notifications, events, stats }: DashboardContentProps) {
  ...
  // Map events and sharedEvents to detailed Event type with all required properties
  ...
  useEffect(() => {
    // Debug: log counts so we can see if provider has the event and the filter matches
  ...
  const handleEventFormSuccess = async () => {
    setShowEventForm(false)
    // Refresh events in-memory so other components pick up changes
    await refresh()
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="w-full sm:max-w-[95%] lg:max-w-[98%] mx-auto px-0 sm:px-3 lg:px-4 py-1 sm:py-3">
        <div className="space-y-1 sm:space-y-3">
          ...
          {/* Main Content Grid */}
          <div className="grid grid-cols-1 gap-1 sm:gap-3 lg:gap-4">
            {/* Full Width Calendar */}
            <div className="px-3 sm:px-0">
            ...
            {/* Upcoming Events - Moved below calendar */}
            <div className="px-3 sm:px-0">
            ...
            {/* Notifications and Personal Schedule - At bottom */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 sm:gap-3 lg:gap-4 px-3 sm:px-0">
              {/* Notifications Panel */}
              <Card className="border-gray-200">
